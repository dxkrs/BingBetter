...
function bing_index(){
    ...
    setTimeout(()=>{
        if(!document.getElementById("logoo")){
            $('.sbox').prepend(divNode)
        }
    },500)
}
...
function bing_search(){
    ...
    setTimeout(()=>{
        //政府tag
        document.querySelector(".sh_favicon").style.setProperty("visibility", "visible", "important");
        const C_list = document.getElementsByClassName("b_title")
        for (let index = 0; index < C_list.length; index++) {
            if(C_list[index].firstChild.href.indexOf(".gov")>0){
                var a = document.createElement("span")
                a.innerText="政府"
                a.className = "tag tag-gov"
                C_list[index].appendChild(a)
            }
        }
        // 更换网站图标
